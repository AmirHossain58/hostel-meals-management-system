...
const MyReviews = () => {
  ...
  const [count, setCount] = useState(0);
    ...
    const {
      ...
      queryKey: ['meals-reviews', user?.email],
      queryFn: async () => {
        const { data } = await axiosSecure.get(`reviews/${user?.email}?page=${currentPage}&size=${itemsPerPage}`)
        return data
      },
    })
    useEffect(() => {
      const getCount = async () => {
        const { data } = await axiosSecure(
          `/user-meals-reviews-count/${user?.email}`
        );
